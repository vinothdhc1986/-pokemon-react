...
function App() {
  ...
  useEffect(() => {
    ...
    setBoxes(newBoxes);
    console.log(`box or level change,${state.boxSize}, ${state.level}  `)
      ...
      <div className="boxes" style={boxesStyle(state.boxSize)}>
        {
          boxes.map((box, i) => {
            return (
              <div key={i} className={`box ${(prev === i || curr === i || completed[box]) ? 'active' : ''} ${completed[box] ? 'completed' : ''}`} 
